...
suggests
\new{the temporal coefficient of variability of ecosystem biomass (\emph{ecosytem variability})}
...
It is notoriously difficult to get more than four species to coexist via relative nonlinearity without introducing some form of fluctuation-independent mechanism like resource partitioning}
(Yuan \& Chesson 2015),
...
then D were \new{added to the initial pool of species}. We then ran
...
fewer species (e.g., lower slopes in log-log space; Fig. SI-8).
...
\new{Moderately asymmetric competition} makes it more difficult for new
...
low. The correlation of species' environmental responses also mediates
the relationship between environmental variance, species richness, and
ecosystem \emph{CV}: lower correlations make it easier for new species
to enter the community and contribute to porfolio effects (Fig. 3).
...
4A; Fig. SI-10). If more stable species colonize, species additions
buffer the ecosystem from increasing environmental variability (Fig.
4B).
...
relationships. For example, Sasaki and Lauenroth (2011) found a positive
...
results in terms of dominant species' effects, it is also possible that
each site experienced sufficiently different levels of environmental
variability to influence species coexistence. DeClerck et al. (2006)
also found a positive diversity--\new{ecosystem variability} when
...
In particular, theory has identified numerous context-dependent processes that determine how much diversity can buffer ecosystem variability}
...
\new{For example, we assumed all species pairs have the same environmental response correlations (e.g., $\rho_{ij} = \rho$), which is clearly not true in nature and can mediate the impact of dominant species on ecosystem variability}
...
But this need not be the case, and increasing environmental variability may threaten small populations}
(Boyce 1992) \new{even as it promotes coexistence of new species} (Adler
\& Drake 2008).
...
Using a very general model,} Thibaut \& Connolly (2013)
...
\new{Recent advances on quantifying the storage effect in natural communities}
(Ellner et al. 2016)
