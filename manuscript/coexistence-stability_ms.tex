...
\new{One limitation of our model is that we did not include demographic stochasticity, which can lead to stochastic extinction for small populations as environmental  variability increases}
(Boyce 1992).
\new{Previous work has shown how demographic stochasticity and coexistence mechanisms can interact to create a "humped-shape" relationship between coexistence time and environmental variability}
(Adler \& Drake 2008).
...
For example, we assumed all species pairs have the same environmental response correlations (e.g., $\rho_{ij} = \rho$), which is clearly not true in nature and can mediate the impact of dominant species on ecosystem variability}
