...
\textbf{\large{Fluctuation-dependent coexistence mechanisms generate positive and negative relationships between species richness and ecosystem stability}}
...
Hints of the idea that diverse ecosystems are more stable through time than species-poor ecosystems go back as far as Darwin [@Darwin1859], who, in passing, mentions the `division of labor' that arises from niche differentiation among species [@Turnbull2013].
...
Model and Analysis
------------------
To discover how fluctuation-dependent coexistence mechanisms affect the diversity-stability relationship, we developed a semi-discrete, mechanistic consumer-resource model that allows coexistence of two species on one resource by either the storage effect or relative nonlinearity. 
...
\noindent $R^+$ is a random resource pulse drawn from a log-normal distribution with mean $\mu(R^+)$ and standard deviation $\sigma(R^+)$. Model parameters and notation are described in table 1.
...
----------------------
The direction of the diversity-variability relationship can be positive and negative (Fig. 2).
