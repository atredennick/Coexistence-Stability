...
ABSTRACT: For biodiversity to increase the temporal stability of ecosystem function in the long-term, coexistence must be stable.
...
*Keywords*: coexistence, storage effect, relative nonlinearity, diversity-stability hypothesis, pulsed differential equation, consumer-resource dynamics, synchrony
...
\begin{align}
  ...
  R(0) &= Z_{t}
\end{align}
\noindent The consumers transition between $N$ and $D$ instantaneously between years. We assume resource density does not change between years. So, at the yearly transition:
\begin{align}
...
$\rho$ & correlation of species' environmental responses (constant) \\
$r$ & daily resource supply pulse (variable) \\
$R_{\mu}$ & mean resource pulse, on log scale (constant) \\
$R_{\sigma^2}^T$ & variance of annual initial resource supply, on log scale (constant) \\
$R_{\sigma^2}^{\tau}$ & variance of daily resource supply, on log scale (constant) \\
