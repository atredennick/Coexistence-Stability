...
Along with resource uptake, consumer population growth depends on the production dormant biomass ($D$), the activation of dormant biomass to live biomass ($D \rightarrow N$), and the survival of living biomass from one year to the next. At the beginning of each season we start with initial conditions defined as $V_{t}$, $W_{t}$, and $Z_{t}$ for the dormant state, the live state, and the resource, respectively. So for each season, eqs. 1-2 are solved given the initial conditions:
...
\end{align}
\noindent where $D(\tau_k)$, $N(\tau_k)$, and $R(\tau_k)$ are the abundances of each state at the end of growing season *k*, $\tau_k^+$ denotes the beginning of growing season $k=1$, and $g$ is a yearly activation rate that regulates how much dormant biomass is converted to growing-season live biomass each year. $R(T^+)$ is a randomly generated resource pulse from a log-normal distribution with mean $R_\mu$ and variance $R_{\sigma^2}$ (same mean and variance used to generate daily pulses). Our formulation assumes that at the end of each season all accumulated living biomass [$N(T^-)$] is converted to dormant biomass. Parameter notations and definitions are in table 1.
...
$R_{\sigma^2}$ & variance of daily resource supply, on log scale (variable) \\
