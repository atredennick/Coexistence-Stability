...
\NormalTok{simulate_model <-}\StringTok{ }\NormalTok{function(seasons, days_to_track, Rmu, }
                           ...
                           \NormalTok{beta1, beta2, beta3, beta4,}
                           \NormalTok{eta1, eta2, eta3, eta4,}
                           \NormalTok{theta1, theta2, theta3, theta4,}
  ...
  \NormalTok{update_DNR <-}\StringTok{ }\NormalTok{function(t, DNR, gammas,}
                         ...
                         \NormalTok{eta1, eta2, eta3, eta4,}
                         \NormalTok{beta1, beta2, beta3, beta4,}
                         \NormalTok{theta1, theta2, theta3, theta4, nu) \{}
    \KeywordTok{with} \NormalTok{(}\KeywordTok{as.list}\NormalTok{(DNR),\{}
      ...
      \NormalTok{N1new <-}\StringTok{ }\NormalTok{beta1*(}\DecValTok{1}\NormalTok{-alpha1)*N1 +}\StringTok{ }\NormalTok{g1*(D1+(alpha1*N1))*(}\DecValTok{1}\NormalTok{-eta1)}
      \NormalTok{N2new <-}\StringTok{ }\NormalTok{beta2*(}\DecValTok{1}\NormalTok{-alpha2)*N2 +}\StringTok{ }\NormalTok{g2*(D2+(alpha2*N2))*(}\DecValTok{1}\NormalTok{-eta2)}
      \NormalTok{N3new <-}\StringTok{ }\NormalTok{beta3*(}\DecValTok{1}\NormalTok{-alpha3)*N3 +}\StringTok{ }\NormalTok{g3*(D3+(alpha3*N3))*(}\DecValTok{1}\NormalTok{-eta3)}
      \NormalTok{N4new <-}\StringTok{ }\NormalTok{beta4*(}\DecValTok{1}\NormalTok{-alpha4)*N4 +}\StringTok{ }\NormalTok{g4*(D4+(alpha4*N4))*(}\DecValTok{1}\NormalTok{-eta4)}
      \NormalTok{Rnew  <-}\StringTok{ }\NormalTok{theta1*(}\DecValTok{1}\NormalTok{-alpha1)*N1 +}\StringTok{ }\NormalTok{theta2*(}\DecValTok{1}\NormalTok{-alpha2)*N2 +}\StringTok{ }
\StringTok{               }\NormalTok{theta3*(}\DecValTok{1}\NormalTok{-alpha3)*N3 +}\StringTok{ }\NormalTok{theta4*(}\DecValTok{1}\NormalTok{-alpha4)*N4 +}\StringTok{ }
\StringTok{               }\NormalTok{nu*R +}\StringTok{ }\NormalTok{Rvector[t]}
  ...
  \NormalTok{getG <-}\StringTok{ }\NormalTok{function(sigE, rho, nTime, num_spp) \{}
    ...
    \NormalTok{if(sigE >}\StringTok{ }\DecValTok{0}\NormalTok{) \{ varcov <-}\StringTok{ }\NormalTok{Matrix::}\KeywordTok{nearPD}\NormalTok{(varcov)$mat \} }\CommentTok{# coerce matrix to PD }
...
\begin{figure}[!ht]
  ...
  \caption{Resource uptake curves for each species (represented by different colors) as used in relative nonlinearity simulations.}
\end{figure}
