...
\begin{figure}
\begin{center}
<<echo=FALSE, fig=TRUE,jpeg=TRUE,height=3, width=8.5 >>=
source("../modelCode/continuousStorageEffect_CR_v4.R")
@
\end{center}
\caption{Example time-series from our pulsed consumer-resource model where $g$ is perfectly uncorrelated (storage effect operating), $r_1$ = \Sexpr{r[1]}, and $r_2$ = \Sexpr{r[2]}. Species 1 is orange, species 2 is purple. The far right panel shows the population, by species, and community-level coefficient of variation.}
\end{figure}

\subsection{Implementing relative nonlinearity}
When considering consumer-resource dynamics, species coexistence by relative nonlinearity requires that each species has different nonlinear responses to resource availability, and resource availability must fluctuate through time. In a constant resource environment, the species with the lowest R* will always exclude the other species.  

\begin{figure}
\begin{center}
<<echo=FALSE, fig=TRUE,jpeg=TRUE,height=3, width=8.5 >>=
source("../modelCode/continuousRelativeNonlinearity_CR.R")
@
\end{center}
