...
Theoretical explorations of the diversity--ecosystem variability relationship over the past three decades have highlighted numerous context-dependent processes that determine how much diversity can buffer ecosystem variability, and we have added another wrinkle.
To make our current analysis tractable, we ignored several potentially important processes and phenomena.
For example, we assumed all species pairs have the same environmental response correlations (e.g., $\rho_{ij} = \rho$), which is clearly not true in nature and can mediate the impact of dominant species on ecosystem variability} [@Thibaut2013].
...
Lastly, we ignored demographic stochasticity, assuming our species had a generous seed source to bounce back from low population density.
But this need not be the case, and increasing environmental variability may threaten small populations even as it promotes coexistence of new species.
This dynamic may be especially relevant to applied work because habitat fragmentation and increasing environmental variability are likely to happen together.} 
