...
Portfolio effects under the storage effect remained strong in an eight-species community, where total community \emph{CV} saturated after the fifth species (Fig. SI-x).
...
In fact, ecosystem \emph{CV} remains relatively constant past four species when species have independent responses to the environment ($\rho=0$; Fig. SI-4).
...
When additional species, which immigrate from the regional pool, are less intrinsically stable than the resident species, ecosystem variability increases at a constant rate even as species are added (Fig. 4A; Fig. SI-10).
...
For the storage effect, this is evident in Fig. 3 where we made live-to-dormant transition rates ($\alpha$s) asymmetric, and we achieved the same result when we made dormant mortality rates ($\eta$s) asymmetric (Fig. SI-x).
Furthermore, increasing the absolute value of dormant mortality also made coexistence more difficult, but results were qualitatively similar to those above.
For relative nonlinearity, altering the absolute value of the mean resource pulse shifts where species can coexist across a gradient of resource variability (Fig. SI-x).
...
These mechanisms can translate increased variability into higher species richness (Fig. SI-2), but the increase in environmental variability also increases ecosystem variability.
\new{We limited our primary analysis to four-species communities, but the buffering effect of additional species under the storage effect is even stronger in more species rich communities (Fig. SI-x).}
...
\new{So far as we know, prior to the work presented here the effect of dominant species on ecosystem variability was the only available theoretical explanation for a positive diversity--\new{ecosystem variability} relationship} [e.g., @Thibaut2013].
@DeClerck2006 also found a positive diversity--\new{ecosystem variability} when sampling conifer richness and the variability of productivity across a large spatial gradient in the Sierra Nevada.
