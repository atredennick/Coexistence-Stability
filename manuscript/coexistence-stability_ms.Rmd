...
This idea underlies the "insurance hypothesis" [@Yachi1999], which suggests \new{the temporal coefficient of variability of ecosystem biomass (\emph{ecosytem variability})} decreases with diversity because species respond dissimilarly to environmental variation, broadening the range of conditions under which the community maintains function [@Loreau2010].
...
In brief, species coexistence via the storage effect occurs when species have buffered population growth to avoid large population declines in ``bad environment'' years (e.g., too dry for seed germination) and there is positive covariance between competition and the environment.
Species coexistence via relative nonlinearity occurs when species have nonlinear and unique responses to some limiting factor that fluctates from year-to-year (e.g., soil moisture).
Importantly, variability in the driving environmental factor promotes species richness in both mechanisms.
...
\new{In his thorough review of the topic, Loreau (2010) cautions that "one of the pieces of the stability jigsaw that is still missing here is the interconnection between community stability and the maintenance of species diversity due to temporal environmental variability."}
...
It is notoriously difficult to get more than four species to coexist via relative nonlinearity without introducing some form of fluctuation-independent mechanism like resource partitioning} [@Yuan2015], \new{which would then cloud our inference on the effect of fluctuation-dependent coexistence.
So, for consistency we also constrain our focus to four species communities under the storage effect, but our conclusions generally apply to more species-rich communities too (see SI section SI.2).}
...
\vspace{-3em}
\begin{align}
D_{i,\tau + 1} = \left[\alpha_i N_{i,\tau(T)} + D_{i,\tau(T)} \right](1 - \eta_i)
\end{align}
\vspace{-3em}
...
\new{In other words, resource uptake is above average in a year when an abundant species experiences high germination rates because combined population size will then be above average, but resource uptake will be below average in a year when a rare species experiences high germination rates because combined population size will be below average.}
...
\new{To do this, we simulated communities with all species initially present across a gradient of annual resource variability for relative nonlinearity (50 values of $\sigma_R$ in the range [0, 1.4]) or environmental cue variability for the storage effect (100 values of $\sigma_E^2$ in the range [0, 3]).}
...
Our focus is specifically on communities where fluctuation-dependent coexistence is operating, and making parameters asymmetric among species typically reduced coexistence strength or made coexistence impossible (Supporting Information section SI.2).
Changes in the absolute values of parameters also altered the strength of coexistence, but in no case did altering parameter values change the qualitative results and conclusions we present below (see Supporting Information section SI.2; Figs. SI-1,SI-2,SI-3).}
...
For relative nonlinearity, total community \emph{CV} decreased with species richness at a given level of environmental variability because additional species increased mean biomass (over-yeilding) and, generally, reduced the temporal standard deviation (Fig. SI-7).
...
While it appears ecosystem \emph{CV} remains constant in a four-species community when $\rho < 0$, it does eventually rise as environmental cue variability continues to increase past the values shwon in Fig. 3A (Fig. SI-9).}
...
<!---
\new{One limitation of our model is that we did not include demographic stochasticity, which can lead to stochastic extinction for small populations as environmental  variability increases} [@Boyce1992].
\new{Previous work has shown how demographic stochasticity and coexistence mechanisms can interact to create a "humped-shape" relationship between coexistence time and environmental variability} [@Adler2008].
\new{This occurs because environmental variability increases coexistence strength and the probability of stochastic extinction simultaneously.
We did not consider this potential effect here because our focus was on large populations that would most influence ecosystem functioning.
However, an interesting extension of our work will be to investigate the role of small populations and demographic stochasticity.}
--->
...
---------------------------
\new{What we have presented here is clearly just a beginning.
Theoretical explorations of the diversity--ecosystem variability relationship assuming fluctuation-independent ceoxistence have a three decade head-start, and we have some catching up to do.
...
\new{This dynamic may be especially relevant to applied work because habitat fragmentation and increasing environmental variability are likely to happen together.} 
