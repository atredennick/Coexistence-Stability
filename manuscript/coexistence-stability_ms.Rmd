...
\vspace{-3em}
\begin{align}
N_i(\tau^+) = \gamma_{i,\tau} D_i(\tau^+),
\end{align}
\vspace{-3em}
\noindent \new{where \emph{i} indexes each species and $\tau^+$ denotes the beginning of growing season $\tau$.}
\new{Live biomass at the start of the growing season ($N_i(\tau^+)$) then serves as the initial conditions for continuous-time} consumer-resource dynamics that are modeled as two differential equations:
\vspace{-3em}
\begin{align}
...
\new{At the end of the growing season, a fraction ($\alpha_i$) of live biomass is stored as dormant biomass and a fraction of dormant biomass survives ($\eta_i$) to the next growing season, giving the following equation:}
\vspace{-3em}
\begin{align}
D_i{\tau + 1} = \left[\alpha_i N_i(\tau^-) + D_i(\tau^-) \right](1 - \eta_i)
\end{align}
\vspace{-3em}
\noindent \new{ where $\tau^-$ denotes the end of growing season $\tau$.
We assume remaining live biomass ($N(\tau^-)(1-\alpha)$) dies (i.e., this is not a closed system where all biomass must be in either $N$ or $D$ states).}

We assume the resource pool is not replenished within a growing season.
Resource replenishment occurs between growing seasons, and the resource pool (*R*) at the start of the growing season is $R(\tau^+) = R^+$, where $R^+$ is a random resource pulse drawn from a log-normal distribution with mean $\mu(R^+)$ and standard deviation $\sigma(R^+)$. 
...
Thus, the initial conditions for each state ($N, D, R$) at the beginning of growing season $\tau+1$ are:}
\vspace{-3em}
\begin{align}
...
\end{align}
\vspace{-3em}
\noindent \new{where, as above, $\tau^-$ denotes the end of growing season $\tau$ and $\tau^+ + 1$ denotes the beginning of growing season $\tau+1$.} 
