...
#mean and standard deviation of annual resource supply
meanR <- 50
Rvar <- c(0,5,10,15,20,25,30)
...
#### Run simulations
####
outStab <- matrix(NA, nrow = length(Rvar), ncol = 3)
for(doSD in 1:length(Rvar)){
  Nstart <- 5
  Nsave <- matrix(nrow = years, ncol=2)
  Nsave[1,] <- c(Nstart,Nstart)
  for(g in 2:years){
    days = 1:maxDays
    NR[3] <- max(0,rnorm(1,meanR,Rvar[doSD]))
    # species parameters 
...
outStab$Rvar <- Rvar 
colnames(outStab) <- c("N1", "N2", "Community", "Rvar")
outM <- melt(outStab, id.vars = "Rvar")

ggplot(outM, aes(x=Rvar, y=value, linetype=variable))+
  geom_line()+
  geom_point()
