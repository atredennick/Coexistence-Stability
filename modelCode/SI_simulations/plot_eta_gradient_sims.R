...
library(ggthemes)
library(dplyr)
results_path       <- "../../simulationResults/SI_results/etas_assymetry_results/"
...
cv_means$eta_fact <- "2asymmetric"
cv_means[which(cv_means$eta==0.125),"eta_fact"] <- "1symmetric"

# Find species first occurences for lines
firstones <- cv_means %>%
  group_by(rho, eta_fact, num_species) %>%
...
ggplot()+
  ...
  facet_grid(eta_fact~rho, scales = "free_y")+
  my_theme+
...
ggsave(paste0(figures_path, "SI_storage_effect_asymmetric_etas.png"), width = 133, height=90, units = "mm", dpi = 600)
