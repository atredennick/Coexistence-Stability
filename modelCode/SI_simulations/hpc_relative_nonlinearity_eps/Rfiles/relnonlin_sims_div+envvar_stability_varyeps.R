...
prm <- expand.grid(as.matrix(rsd_vec), 1:2, 1:4)

DNR_repped <- matrix(rep(DNR,each=(nrow(prm)/nrow(DNR))),ncol=ncol(DNR))
eps_repped <- matrix(rep(eps,each=23),ncol=ncol(eps))

colnames(prm) <- c("Rsd_annual", "epsid", "dnr_id")
...
saveRDS(outs, paste0("relnonlin_div_envvar_stability_hiloweps_", i, ".RDS"))