...
#storage vectors
cvOut <- numeric(length(Rsd))
cvRes <- numeric(length(Rsd))
meanOut <- numeric(length(Rsd))
sdOut <- numeric(length(Rsd))
sppRatio <- numeric(length(Rsd))
...
#### Plot results
####
cvD <- data.frame(sVar=Rsd,CV=cvOut,meanN=meanOut,sdN=sdOut,sppRatio=sppRatio)
g1 <- ggplot(cvD, aes(x=sVar, y=CV))+
  geom_line()+
...
g2 <- ggplot(cvD, aes(x=sVar, y=meanN))+
  geom_line()+
  geom_point(size=8, color="white")+
  geom_point(size=4)+
  xlab(expression(sigma[R]))+
...
g3 <- ggplot(cvD, aes(x=sVar, y=sdN))+
  geom_line()+
  geom_point(size=8, color="white")+
  geom_point(size=4)+
  xlab(expression(sigma[R]))+
...
g4 <- ggplot(cvD, aes(x=sVar, y=sppRatio))+
  geom_line()+
  geom_point(size=8, color="white")+
  geom_point(size=4)+
